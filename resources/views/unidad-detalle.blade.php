...
<body class="bg-dark">
  ...
  <div class="container mt-5 fade-in">
    <div class="row justify-content-center">
      <div class="col-md-8 text-center">
        <div class="image-gallery">
          ...
          <img loading="lazy" src="data:image/jpeg;base64,{{ $unidad->imagenes[0] }}" alt="Imagen principal" class="mb-3 img-fluid" id="mainImage" onclick="openMainImageModal()" data-bs-toggle="modal" data-bs-target="#mainImageModal">
          <div class="d-flex justify-content-center gap-2">
            @foreach ($unidad->imagenes as $index => $imagen)
              @if ($index < 4)
    ...
    <div class="product-description mt-4">
      <h5 class="titles2" style="text-align: center;">Descripción de la unidad</h5> <br>
      ...
      <tbody>
          ...
          <tr>
            ...
            <td>
              @if (!empty($unidad->documentacion) && is_array($unidad->documentacion))
                @foreach ($unidad->documentacion as $index => $documento)
                  <a href="data:application/pdf;base64,{{ $documento }}" target="_blank">Ver Documento {{ $index + 1 }}</a><br>
                @endforeach
              @else
                No disponible
              @endif
          ...
          <tr>
            <th scope="row">Diferencial</th>
            <td>{{ $unidad->diferencial ?? 'No disponible' }}</td> <!-- Campo dinámico agregado -->
          </tr>
          <tr>
            <th scope="row">Tipo de Camarote</th>
            <td>{{ $unidad->tipo_camarote ?? 'No disponible' }}</td> <!-- Campo dinámico agregado -->
          </tr>
          <tr>
          ...
          <tr>
            <th scope="row">Ejes Delanteros</th>
            <td>{{ $unidad->ejes_delanteros }}</td>
          </tr>
          <tr>
            <th scope="row">Ejes Traseros</th>
            <td>{{ $unidad->ejes_traseros }}</td>
          </tr>
        
          <tr>
            <th scope="row">Precio</th>
            <td>${{ number_format($unidad->precio, 2) }}</td>
          </tr>
          <tr>
            <th scope="row">Datos Adicionales</th>
            <td>{{ $unidad->datos_adicionales }}</td>
          </tr>
        </tbody>
      </table>
    ...
    onclick="window.location.href=`https://api.whatsapp.com/send?phone=+526146056639&text=Quiero%20más%20información%20de%20esta%20unidad:%20${encodeURIComponent(window.location.href)}`">
    ...
    <div class="modal-dialog modal-dialog-centered modal-lg">
      <div class="modal-content">
        ...
        <div class="modal-body text-center">
          <img src="data:image/jpeg;base64,{{ $unidad->imagenes[0] }}" alt="Imagen principal ampliada" class="img-fluid enlarged-image" id="mainImageEnlarged">
        </div>
      </div>
    </div>
  </div>
      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
          <div class="grid-gallery">
            @foreach ($unidad->imagenes as $imagen)
 ...
 function printTable() {
  if (window.matchMedia("only screen and (max-width: 760px)").matches) {
    ...
    const doc = new jsPDF();
    doc.text("Detalles de la Unidad", 10, 10);
      ...
      .then(() => {
        ...
        if (navigator.share) {
          navigator.share({
            ...
            text: 'Mira esta unidad en venta:',
            url: url
          })
